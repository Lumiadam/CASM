...
export default function MaintenancePage() {
  ...
  return <RequireAuth><AppShell><div className="space-y-6">
    ...
    <section className="space-y-3"><h3 className="font-semibold">工單追蹤</h3>{orders.length === 0 ? <p className="text-sm text-[rgb(var(--muted))]">尚無工單。</p> : orders.map((order) => <article key={order.id} className="card-panel p-4"><div className="flex flex-wrap justify-between gap-2"><div><p className="font-medium">#{order.id} {order.asset_name}</p><p className="text-sm">{order.issue_type} · {order.severity}</p></div><span className="rounded bg-slate-500/10 px-2 py-1 text-sm">{order.status}</span></div><p className="mt-2 text-sm">{order.description}</p><p className="mt-2 text-xs text-[rgb(var(--muted))]">申報人：{order.reporter_name || "-"}　零件：{order.parts.map((part) => `${part.name} × ${part.quantity}`).join("、") || "尚無"}</p>{role !== "Employee" && <div className="mt-3 flex flex-wrap gap-2"><button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setStatus(order.id, "IN_REPAIR")}>開始維修</button><button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setStatus(order.id, "WAITING_PARTS")}>等待零件</button><button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setStatus(order.id, "VERIFICATION")}>驗證</button><button type="button" className="rounded border px-2 py-1 text-xs" onClick={() => setStatus(order.id, "CLOSED")}>結案</button></div>}</article>)}</section>
